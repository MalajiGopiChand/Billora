...
export function Login() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(''); setBusy(true); try { await login(email, password); navigate('/'); } catch { setError('Unable to sign in. Check your email and password.'); } finally { setBusy(false); } };
