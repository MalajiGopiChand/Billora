...
export function Login() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(''); setBusy(true); try { await login(email, password); navigate('/dashboard'); } catch (err) { setError(authErrorMessage(err, 'Unable to sign in. Check your email and password.')); } finally { setBusy(false); } };
