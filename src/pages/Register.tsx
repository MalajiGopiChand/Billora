...
export function Register() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(''); if (password.length < 6) { setError('Use at least 6 characters for the password.'); return; } setBusy(true); try { await register(email, password); navigate('/'); } catch { setError('Unable to create account. This email may already be in use.'); } finally { setBusy(false); } };
